...
try {
  ...
  console.error("Build failed! Might need to run 'npm install' first.", error);
  process.exit(1);
}
...
try {
  execSync(
    `npx @yao-pkg/pkg . --targets node20-linux-x64,node20-win-x64,node20-macos-x64 --output ${BUILD_DIR}/${SCRIPT_NAME}`,
    { stdio: "inherit" },
  );
} catch (error) {
