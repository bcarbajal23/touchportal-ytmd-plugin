...
import { TPClient } from "../touchPortalClient";
import { TP_ACTIONS, TP_STATES } from "./tpConstants";
import { PlayerState } from "../../types/ytmd";
import { ActionId, ActionData } from "../../types/touchPortal";

export type ActionHandler = (data: ActionData) => Promise<void> | void;
  ...
  return {
    ...
    [TP_ACTIONS.ytmdShuffleMode]: async () => {
      ...
      tpClient.stateUpdate(
        TP_ACTIONS.ytmdShuffleMode,
        playerState.currentShuffleMode,
      );
      tpClient.stateUpdate(
        TP_STATES.ytmdShuffleMode,
        playerState.currentShuffleMode,
      );

      await ytmdClient.restClient.shuffle();
    },
    [TP_ACTIONS.ytmdLikeDislike]: async (data: ActionData) => {
      // Grab the current like/dislike state of the current song/video
      const actionLikeDislikeState = data.data.find(
        (item) => item.id === "ytmd.choice.likeDislike",
      )?.value;
